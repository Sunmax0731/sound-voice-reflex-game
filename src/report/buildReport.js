...
export function buildQcdsArtifacts() {
  ...
  writeJson("docs/qcds-regression-baseline.json", { product: product.repo, baselineVersion: "v0.1.0-alpha.1", representativeScenarios: ["happy-path","missing-required","warning","mixed-batch"], expectedValidationResult: "pass", expectedWebSmokeResult: "pass", manualTestStatus: "not-executed", releaseAssetContract: ["dist/sound-voice-reflex-game-docs.zip","docs/manual-test.md","docs/strict-manual-test-addendum.md"] });
}
...
export function buildDocsZip() { const missing = docsEntries.filter((x) => !fs.existsSync(path.join(repoRoot, x))); if (missing.length) throw new Error("Missing docs ZIP entries: " + missing.join(", ")); createDeterministicZip(repoRoot, docsEntries, path.join(repoRoot, "dist/sound-voice-reflex-game-docs.zip")); }
